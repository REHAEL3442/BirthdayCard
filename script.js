 const FORMSPREE_URL = 'https://formspree.io/f/mkolqqny'; // <-- REPLACE THIS with your form URL
  ...
  const correctPW = ['07/04/2011','07/21/2008','october/04/2009','10-04-2009','october 4 2009','10/4/2009'];
  ...
  function runWarpCanvas(){
    ...
    const COLS=[
      ...
      '255,255,255','248,248,255','255,255,255',
      '255,185,225','255,165,215','240,180,255',
      '200,170,255','215,185,255',
      '255,225,135','255,215,105',
      '175,238,255','160,228,252',
    ];
    ...
    function getSpeed(p){
      ...
      if(p<0.58) return 1680 + Math.sin(p*12)*28;
      const d=(p-0.58)/0.42;
  ...
  function initGalaxy3D(){
    ...
    G3D.hitSpheres   = [];
    const mobile = W < 600;
    const N  = mobile ? 4000 : 7500;
    const CN = mobile ?  450 :  900;
  ...
  submitBtn.addEventListener('click', async ()=>{
    ...
    const galaxyName = currentGalaxy ? GDEFS.find(d => d.id === currentGalaxy)?.name || 'unknown' : 'unknown';
    
    const formData = new FormData();
    formData.append('answer', selectedAnswer === 'yes' ? 'YES 💖' : 'NO 💔');
    formData.append('feedback', feedbackBox.value.trim() || '(no message)');
    formData.append('galaxy', galaxyName);
    formData.append('_subject', '💌 Birthday Court Answer!');
    ...
    try {
      ...
      courtStatus.textContent = "Couldn't send right now, but your answer is still in my heart 💗";
      showThanks(selectedAnswer);
    }
  });
